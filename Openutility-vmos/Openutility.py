...
class BootAnimation:
    ...
    def fade_in(self, item, duration=1000, delay=0):
        ...
        if isinstance(item, int):  # 如果是canvas对象
...
class Openutility:
    ...
    def __init__(self, root):
        ...
        # 显示开机动画
        BootAnimation(root, self.on_boot_complete)

    def on_boot_complete(self):
        '''开机动画完成后的回调函数 '''
        #显示主窗口并初始化系统
        self.root.deiconify()
        self.root.title('Zero OS 1.0')
    ...
    def create_taskbar(self):
        ...
        # 电量显示
        self.battery_icon = tk.Label(self.tray_frame, text=f' no% ',
                                   bg='#1a1a1a', fg='white',font=('Segoe UI', 10))
        self.battery_icon.pack(side=tk.RIGHT, padx=2)
        # 锁屏按钮
    ...
    def create_window(self, title, content):
        ...
        # 将窗口添加到任务栏
        self.add_to_taskbar(title, window)
    ...
    def update_clock(self):
        ...
        每秒自动更新一次
        '''
        self.tray_icon.config(text=time.strftime("%H:%M:%S"))  #更新时钟文本
    ...
    def shutdown(self):
        '''关机'''
        if messagebox.askyesno('Zero系统', '确定关机'):#选择框
            time.sleep(2)
            sys.exit()

    def restart(self):
        '''重启'''
        if messagebox.askyesno('Zero系统', '系统重启'):
            time.sleep(1.6)
            self.root.destroy()
            #模拟重启
    ...
    def lock_screen_func(self):
        ...
        # 密码输入框
        pw_frame = tk.Frame(lock_content, bg='blue')
